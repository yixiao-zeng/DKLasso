...
def hierar_prox(v, u, *, lambda_, lambda_bar, M):
    """
    v has shape (m,) or (m, batches)
    u has shape (k,) or (k, batches)
    beta_star: skip layer weights
    theta_star: first hidden layer weights
    """
...
def inplace_prox(beta, theta, lambda_, lambda_bar, M):
    beta.weight.data, theta.weight.data = hierar_prox(
